...
def inspect():
    ...
    try:
        records = list(db.call_verification_records.find().sort("call_timestamp", -1))
        ...
        if not records:
            ...
            # Show "Columns" (Keys)
            latest = records[0]
            print(f"  Existing Columns (Fields): {list(latest.keys())}")
            print("-" * 40)
        ...
        for r in records[:5]: # Show last 5
            ...
            print(f"  Embedding: {'Yes' if r.get('voice_embedding') else 'No'}")
            print("-" * 40)
    except Exception as e:
